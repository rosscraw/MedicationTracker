        ...
        builder: (context, medicationList) {
          if(medicationList.connectionState == ConnectionState.waiting) {
            ...
            return ListView.builder(
                ...
                shrinkWrap: true,
                itemCount: user.getMedicationList().length,//medicationList.data.length,//userSnapshot.data['medication'].length,
                itemBuilder: (context, index) {
                  return Card(
                    child: ListTile(
                      leading: Icon(user.getMedicationList()[index]
                          .getMedication()
                          .getMedicationIcon()),
//                //TODO link to database
                      title: Text(
                        user.getMedicationList()[index].getMedication().getName(),
                        style: TextStyle(
                          fontSize: 20.0,
                        ),
                      ),
                      subtitle: Text(
                        user.getMedicationList()[index].getDosage() + user.getMedicationList()[index].getDosageUnits(),
                        style: TextStyle(
                          fontSize: 15.0,
                        ),
                      ),
                          ...
                          FlatButton.icon(
                            ...
                            onPressed: () {
                              navigateToMedicationDetails(
                                  user.getMedicationList()[index], user);
                            },
                          ),
                        ],
                      ),
